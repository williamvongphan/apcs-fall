...
public class Bankaccount {
	...
	public int setAcctNum(int newAcctNum) {
		...
		if (!(newAcctNum < 999999999 && newAcctNum > 99999999)) {
			acctNum = 999999999;
			System.out.println(
					"Your account number you provided was invalid (not a 9 digit number), so we set it to 999999999");
			return oldAcctNum;
		} else {
			acctNum = newAcctNum;
			return oldAcctNum;
	...
	// Notice that we overloaded the function to take either a pin or a password.
	// It's good that the pin and the password use different data types.

	// The following function(s) should really be private, but are public for the
	// testing purposes.
	...
	public boolean checkAuth(String userPasswd) {
		return userPasswd != passwd ? true : false;
	}
